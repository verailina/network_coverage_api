...
from network_coverage_api.api.schemas import (
    Operator, NetworkCoverage, NetworkCoverageDetailed, Location, Address)

client = TestClient(app)


@pytest.mark.parametrize("coverage, response_json",
                         [
...
@pytest.mark.parametrize("coverage, response_json",
                         [
                             ([NetworkCoverageDetailed(
                                 operator=Operator.SFR, N2G=True, N3G=False, N4G=True,
                                 distance="0.5",
                              ...
                              [{"operator": "SFR", "2G": True, "3G": False, "4G": True,
                                "distance": 0.5,
                                "target_location": {"latitude": 1, "longitude": 1, "address": "Paris"},
                                "closest_location": {"latitude": 1, "longitude": 1, "address": "Paris"}}]),
                             ([], [])
                         ])
@patch('network_coverage_api.api.network_coverage_router._get_network_coverage')
...
@pytest.mark.parametrize("geocoded_value, closest_data, detailed",
                         [(None, None, False),
                          (Mock(), None, False),
                          (Mock(), Mock(), False),
                          (Mock(), Mock(), True)
...
def test__get_network_coverage(
        location_mock, detailed_nc_mock, nc_mock, point_mock, geocode_reverse_mock, geocode_mock,
        searcher_mock, data_mock, geocoded_value, closest_data, detailed):
    ...
    if geocoded_value is None:
        ...
        searcher.find_closest_point_data.assert_called_with(point_mock.return_value, data)
        ...
        if closest_data is None:
            ...
            geocode_reverse_mock.assert_called_once_with(closest_data.point.latitude, closest_data.point.longitude)
